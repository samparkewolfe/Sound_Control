...
function add_finger_to_right_hand()
{
	...
	{
		right_hand_data = right_hand_data.concat(input_list.slice(3, input_list.length));
...
function add_finger_to_left_hand()
{
	if(input_list[2] == "distal")
	{
		left_hand_data = left_hand_data.concat(input_list.slice(3, input_list.length));
...
function bang()
{
	...
	flattened_data = right_hand_data.concat(left_hand_data);
	//post("Final data length: " + flattened_data.length + "\n");
	outlet(0, flattened_data);
...
{
	for(var i = 0; i < 69; i++) left_hand_data[i] = 0;
}

function fill_right_hand()
{
	for(var i = 0; i < 69; i++) right_hand_data[i] = 0;
}
